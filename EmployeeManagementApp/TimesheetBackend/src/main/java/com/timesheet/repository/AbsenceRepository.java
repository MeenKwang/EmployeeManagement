...
public interface AbsenceRepository extends JpaRepository<Absence, Integer> {
    @Query("SELECT new com.timesheet.dto.AbsenceDto(absence.id, absence.reason, absence.employee.id, absence.absenceType.id, absence.typeOff.id, absence.dateRequest, absence.dateSubmit, absence.typeTimeOff,absence.timeOff) " +
    ...
    @Query("SELECT new com.timesheet.dto.AbsenceDto(absence.id, absence.reason, absence.employee.id, absence.absenceType.id, absence.typeOff.id, absence.dateRequest, absence.dateSubmit, absence.typeTimeOff, absence.timeOff) FROM Absence absence WHERE absence.dateRequest = ?1")
    ...
    @Query("SELECT new com.timesheet.dto.AbsenceViewDto(absence.id, absence.absenceType.name, absence.typeTimeOff, " +
            "CASE WHEN typeOff IS NULL THEN 'NONE' ELSE typeOff.name END, absence.timeOff, absence.reason, absence.status) " +
    ...
    List<AbsenceViewDto> listAllPendingAbsenceOfStaffInParticularMonth(int staffId, int month, int year);
    @Modifying
